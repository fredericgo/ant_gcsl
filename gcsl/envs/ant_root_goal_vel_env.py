...
class AntRootGoalVelEnv(goal_env.GoalEnv):
    ...
    def __init__(self):
        super(AntRootGoalVelEnv, self).__init__()
        self.env = Env()
    ...
    def _reward_function(self, obs):
        ...
        h_reward = np.exp(-1 * h_diff)
        q_reward = np.exp(-1 * q_diff)

        return .8 * q_reward + .2 * h_reward
