...
from gcsl.common.util import axis_angle_to_quaternion, quaternion_multiply, quaternion_apply
import torch
...
class Skeleton:
    ...
    def get_joint_locations(self, s):
        ...
        results = []
        r0 = torch.zeros(q[0].shape[:-1] + (3,), dtype=torch.float32)
        results.append(r0)
        ...
        for i, pi in self._topology:
            ...
            p = torch.as_tensor(self._offsets[i])
