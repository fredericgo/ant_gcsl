...
class GCSL:
    ...
    def train(self):
        ...
        if self.log_tensorboard:
            self.summary_writer = SummaryWriter(osp.join(self.log_dir, 'tensorboard'))
                ...
                if timesteps_since_eval >= self.eval_freq:
                    ...
                    if self.log_dir:
                        ...
                        torch.save(
                            ...
                            osp.join(self.log_dir, 'policy%s.pkl'%modifier)
                        )
                        if hasattr(self.replay_buffer, 'state_dict'):
                            with open(osp.join(self.log_dir, 'buffer%s.pkl'%modifier), 'wb') as f:
                                pickle.dump(self.replay_buffer.state_dict(), f)

                        full_dict = dict(env=self.env, policy=self.policy)
                        with open(osp.join(self.log_dir, 'params%s.pkl'%modifier), 'wb') as f:
